...
export default function ProfessorsPage() {    
    ...
    const deleteTheProfessor = async (professorId: string) => {
        const professors = await deleteProfessor(professorId);
        setProfessors(professors as Professor[]);
    }
    ...
    return (    
        <div>
            <div className="flex items-center justify-between">
                ...
                <NavLink to="/professors/new" className="rounded-lg bg-sky-600 px-4 py-2 text-white font-medium hover:bg-sky-700 transition">
                Add Professor
                </NavLink>
            </div>
            <Table columns={professorColumns} data={professors} renderActions={renderActions}/>
        </div>
    );
}